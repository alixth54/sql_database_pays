...
class DAO {
	...
	*/
	public function getCountries($mot,$selection) {
		$sql="SELECT id_pays, libelle_pays AS 'libelle', population_pays AS 'pop',taux_natalite_pays AS 'tauxnat', taux_mortalite_pays AS 'tauxmort', esperance_vie_pays AS 'esper', taux_mortalite_infantile_pays AS 'tauxmortinf', nombre_enfants_par_femme_pays AS 'enf', taux_croissance_pays AS 'tauxcroi', population_plus_65_pays AS '+65',capitale,superficie FROM `t_pays`";
		if ($selection) {
			if (is_numeric($selection)) {
				$sql.=" WHERE ".$mot."_id=".$selection;
			} else {
				$sql.=" INNER JOIN t_".$mot."s ON (t_".$mot."s.id_".$mot."=t_pays.".$mot."_id) WHERE libelle_".$mot." LIKE '".$selection."'";
			}
	...
	public function getMonde() {//union permet de lié 2 requete sql si meme nombre de colonnes et meme nom de colonne
		$sql="(SELECT libelle_continent AS 'libelle',SUM(population_pays) AS 'pop',AVG(taux_natalite_pays) AS 'tauxnat', AVG(taux_mortalite_pays) AS 'tauxmort',AVG(esperance_vie_pays) AS 'esper',AVG(taux_mortalite_infantile_pays) AS 'tauxmortinf',AVG(nombre_enfants_par_femme_pays) AS 'enf',AVG(taux_croissance_pays) AS 'tauxcroi',SUM(population_plus_65_pays) AS '+65' FROM `t_pays` INNER JOIN t_continents ON (t_continents.id_continent=t_pays.continent_id) GROUP BY libelle_continent ORDER BY libelle_continent) UNION (SELECT \"Total\" AS 'libelle',SUM(population_pays) AS 'pop',AVG(taux_natalite_pays) AS 'tauxnat',AVG(taux_mortalite_pays) AS 'tauxmort',AVG(esperance_vie_pays) AS 'esper',AVG(taux_mortalite_infantile_pays) AS 'tauxmortinf',AVG(nombre_enfants_par_femme_pays) AS 'enf',AVG(taux_croissance_pays) AS 'tauxcroi',SUM(population_plus_65_pays) AS '+65' FROM t_pays);";
		return $this->getResults($sql);
	
	}
	
	public function getRegions($continent="") {
		$sql="SELECT `id_region`, `libelle_region` AS 'libelle', `continent_id` FROM `t_regions`";
